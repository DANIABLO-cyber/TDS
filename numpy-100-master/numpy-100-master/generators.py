...
def ktx_to_dict(input_file, keystarter="<"):
    """Parsing keyed text to a Python dictionary. Interpreta '\\' como salto de línea."""
    answer = dict()
    ...
    for line in lines:
        ...
        if k:
            # Sustituye '\\' por salto de línea real
            answer[k] = val.strip().replace("\\\\", "\n")
...
QHA = ktx_to_dict(os.path.join("source", "exercises100Test.ktx"))
...
def create_markdown(
    destination_filename="100_Numpy_exercises", with_hints=False, with_solutions=False
):
    # Create file name
    if with_hints:
